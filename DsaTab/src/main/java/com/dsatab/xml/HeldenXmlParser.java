...
public class HeldenXmlParser {
    ...
    protected static void fillTalents(Hero hero, Element heroElement) {
        ...
        for (int i = 0; i < talentList.size(); i++) {
            ...
            TalentType talentType = TalentType.byXmlName(talentName);
            int talentValue = Util.parseInt(element.getAttributeValue(Xml.KEY_VALUE));
            if (talentType == null) {
                Debug.w("Unknown Talent Type " + talentName);
            } else {
            ...
            if (talent == null) {
                if (talentType != null)
                    ...
                    talent = new Talent(hero, talentName);
            }
        ...
        for (Element combatElement : combatAttributesList) {
            ...
            if (talentType!=null) {
                ...
                CombatMeleeTalent combatTalent = new CombatMeleeTalent(hero,talentType , at, pa);

                hero.addTalent(combatTalent, false);
            } else {
                Debug.w("Unkown Combat Talent Type "+talentName);
            }
        }
    }
